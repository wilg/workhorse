...
**Stay on target through interruptions.** When the user sends messages mid-execution, capture anything useful — add it to the plan, the item's open questions, or the board — then resume working. Treat the approved plan as your anchor. The user may be thinking out loud, flagging something for later, or discussing the next item. Capture it in the right place and keep going. The things that warrant switching away are: an explicit instruction to stop or change tasks, or finishing the current work unit. Everything else gets noted and you return to the plan.
